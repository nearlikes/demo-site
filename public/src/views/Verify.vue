    ...
    <div class="left">
      ...
      <div class="input">
        <input
          ...
          v-model="code"
        />
        <div v-if="time !== 0" class="timer">
          Resend after {{ time }} seconds
        </div>
      </div>
      <div class="btn">
        <button ref="resend" disabled class="secondary" @click="resend">
          RESEND
        </button>
        <button @click="submit">VERIFY</button>
      </div>
      <h3>
        <router-link to="/login">Change number</router-link>
      </h3>
      <div v-if="error" class="err">{{error}}</div>
    </div>
    <div class="right">
      <lottie-player
        :src="require('../assets/circle.json')"
